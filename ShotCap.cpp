...
// Table id 0 — "modern" CoC font, calibrated against tests/coc1.png
// (post-Supercell graphics update; pure-white digits).
static const OcrFP g_digit_table_modern[] = {
...
// Table id 1 — "pre-change" CoC font (BlueStacks-era, cream-tinted
// digits).  Generated by -ocr_build against tests/coc_B.png (1720x998,
...
static const OcrFP g_digit_table_pre[] = {
    ...
    { 4, {2,3,1},    {14,14,14}, {0x020400, 0x050904, 0x050902}, 13 },
    { 5, {9,8,9},    {15,6,6},   {0x565A57, 0x242725, 0x131713}, 13 },
    { 6, {10,12,8},  {7,10,3},   {0xA4A4A5, 0x908F91, 0x606462}, 13 },
    { 7, {9,8,9},    {13,13,14}, {0x0F170B, 0x091007, 0x242C1D}, 13 },
    { 8, {6,5,6},    {3,3,2},    {0x515250, 0x1F201F, 0xA1A3A1}, 13 },
    { 9, {2,6,11},   {10,11,13}, {0x99999C, 0x939796, 0x9AA38E}, 13 },
};
...
static int ocrDigitAt(int bx, int by, int tol)
{
    ...
    const OcrTable& tbl = g_tables[g_active_table];
    for (int i = 0; i < tbl.count; ++i) {
        const OcrFP& fp = tbl.entries[i];
        for (int j = 0; j < 3; ++j) {
            int xj = bx + kJitter[j];
            bool all_ok = true;
            for (int p = 0; p < 3; ++p) {
...
static int runOcrScanMode(const std::wstring& filePath, const std::string& spec, int tol)
{
    ...
    printf("[OCR_SCAN] rect=(%d,%d)-(%d,%d) tol=%d table=%d (id=%d \"%s\")\n",
        x1, y1, x2, y2, tol, g_tables[g_active_table].count,
...
static int runOcrTestMode(const std::wstring& filePath, const std::string& spec, int tol)
{
    ...
    g_insp.imgW = g_insp.image->GetWidth();
    g_insp.imgH = g_insp.image->GetHeight();
    printf("[OCR_TEST] tol=%d table=%d digits (id=%d \"%s\")\n",
        tol, g_tables[g_active_table].count,
        g_active_table, g_tables[g_active_table].name);
    int rows = 0, passes = 0;
    std::istringstream iss(spec);
    std::string token;
...
// (an OcrFP[] entry to add to g_tables) and asm (cf22's ocr_table) form.  Heuristic: per
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (arg == "-ocr_test_file" && i + 1 < argc)
        {
            // Mirror of -ocr_build_file for -ocr_test specs.
            std::ifstream fin(argv[i + 1]);
            if (!fin) {
                fprintf(stderr, "[ERROR] -ocr_test_file: cannot open %s\n", argv[i + 1]);
                return 1;
            }
            std::stringstream ss;
            ss << fin.rdbuf();
        ...
        {
            // Select which baked fingerprint table -ocr_test / -ocr_scan
            // run against.  Defaults to 0 (modern).  Validated against
            // g_table_count to keep the index in range.
            int id = std::atoi(argv[i + 1]);
            if (id < 0 || id >= g_table_count) {
                fprintf(stderr, "[ERROR] -table %d out of range (have %d table(s): ",
                    id, g_table_count);
                for (int t = 0; t < g_table_count; ++t)
                    fprintf(stderr, "%s%d=%s", t ? ", " : "", t, g_tables[t].name);
                fprintf(stderr, ")\n");
                return 1;
            }
            g_active_table = id;
