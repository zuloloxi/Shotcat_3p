...
// digits).  Trained on a composite of tests/coc_B.png + tests/coc_A.png
// (38 labels total, 19 per image) so the variance filter rejects
// offsets that drift between captures of the same digit.  Single-image
// calibration left several offsets brittle — read fine on the source
// image, missed by 30-40 channel units on a sibling capture.  Self-test
